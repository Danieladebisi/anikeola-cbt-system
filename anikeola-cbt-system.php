...
// --- CBT Question Custom Post Type and Taxonomies (No changes from Version 2.3) ---
/**
...
// Taxonomies (Subject, Class Level, Topic) - No changes from Version 2.3
...
// --- Meta Box for Question Answers (No changes from Version 2.3) ---
...
// --- Meta Box for Exam Settings - UPDATED for Number of Questions to Display ---
...
function anikeola_cbt_exam_settings_meta_box_callback( $post ) {
    ...
    $num_questions_to_display = get_post_meta( $post->ID, '_anikeola_cbt_num_questions_to_display', true );
    ?>
    <table class="form-table">
        <tbody>
            ...
            <tr>
                <th scope="row">
                    <label for="anikeola_cbt_num_questions_to_display"><?php esc_html_e( 'Number of Questions to Display', 'anikeola-cbt' ); ?></label>
                </th>
                <td>
                    <input type="number" id="anikeola_cbt_num_questions_to_display" name="_anikeola_cbt_num_questions_to_display" value="<?php echo esc_attr( $num_questions_to_display ); ?>" min="0" step="1" />
                    <p class="description"><?php esc_html_e( 'Enter the number of questions to randomly select from the assigned questions for this exam. Leave blank or 0 to use all assigned questions.', 'anikeola-cbt' ); ?></p>
                </td>
            </tr>
        </tbody>
    </table>
...
function anikeola_cbt_save_exam_settings_meta_box_data( $post_id ) {
    ...
    if ( isset( $_POST['_anikeola_cbt_num_questions_to_display'] ) ) { update_post_meta( $post_id, '_anikeola_cbt_num_questions_to_display', sanitize_text_field( $_POST['_anikeola_cbt_num_questions_to_display'] ) ); }
}
...
function anikeola_cbt_enqueue_front_end_assets() {
    ...
    if ( $load_assets ) {
        wp_enqueue_style('anikeola-cbt-front-style', plugin_dir_url( __FILE__ ) . 'public/css/anikeola-cbt-front.css', array(), '2.3');
        wp_enqueue_script('anikeola-cbt-front-script', plugin_dir_url( __FILE__ ) . 'public/js/anikeola-cbt-front.js', array( 'jquery' ), '2.3', true );
...
function anikeola_cbt_admin_enqueue_scripts( $hook_suffix ) {
    ...
    if ( ( $hook_suffix == 'post-new.php' || $hook_suffix == 'post.php' ) && isset($post_type) && $post_type == 'cbt_exam' ) {
        wp_enqueue_style('anikeola-cbt-admin-style', plugin_dir_url( __FILE__ ) . 'admin/css/anikeola-cbt-admin.css', array(), '2.3');
        wp_enqueue_script('anikeola-cbt-admin-exam-script', plugin_dir_url( __FILE__ ) . 'admin/js/anikeola-cbt-admin-exam.js', array( 'jquery', 'jquery-ui-sortable' ), '2.3', true );
...
function anikeola_cbt_exam_shortcode( $atts ) { 
    ...
    $all_question_ids = get_post_meta( $exam_id, '_anikeola_cbt_exam_question_ids', true );
    if ( ! is_array( $all_question_ids ) || empty( $all_question_ids ) ) { return '<p>' . esc_html__( 'This exam currently has no questions configured.', 'anikeola-cbt' ) . '</p>'; }

    $num_questions_to_display = get_post_meta( $exam_id, '_anikeola_cbt_num_questions_to_display', true );
    $num_questions_to_display = !empty($num_questions_to_display) && is_numeric($num_questions_to_display) ? intval($num_questions_to_display) : 0;

    $question_ids_for_this_exam = $all_question_ids; // Start with all assigned questions

    $randomize_questions = get_post_meta( $exam_id, '_anikeola_cbt_randomize_questions', true );
    if ( $randomize_questions === '1' ) {
        shuffle( $question_ids_for_this_exam );
    }
    
    // If a specific number of questions is set, slice the array
    if ( $num_questions_to_display > 0 && $num_questions_to_display < count($question_ids_for_this_exam) ) {
        $question_ids_for_this_exam = array_slice( $question_ids_for_this_exam, 0, $num_questions_to_display );
    }
    
    if ( empty( $question_ids_for_this_exam ) ) { // Double check after potential slicing
        return '<p>' . esc_html__( 'Not enough questions available for this exam configuration.', 'anikeola-cbt' ) . '</p>';
    }
    ...
    <div class="anikeola-cbt-exam-wrapper" id="anikeola-cbt-exam-<?php echo esc_attr( $exam_id ); ?>" data-exam-id="<?php echo esc_attr( $exam_id ); ?>">
        ...
        <?php if ( $time_limit_seconds > 0 ) : ?>
            <div class="anikeola-cbt-timer">
                ...
                <div class="anikeola-cbt-progress-container">
                    ...
                    <div class="anikeola-cbt-progress-text">0 / <?php echo count($question_ids_for_this_exam); ?> Answered</div>
                </div>
            </div>
        <?php endif; ?>
        ...
        <form id="anikeola-cbt-exam-form-<?php echo esc_attr( $exam_id ); ?>" class="anikeola-cbt-exam-form" method="post">
            ...
            <div class="anikeola-cbt-questions-container">
                ...
                foreach ( $question_ids_for_this_exam as $index => $question_id ) :
                ...
                ?>
                    <div class="anikeola-cbt-question" id="question-<?php echo esc_attr($exam_id . '-' . $question_id); ?>" data-question-id="<?php echo esc_attr($question_id); ?>">
                        ...
                        <ul class="anikeola-cbt-answer-options">
                            <?php 
                            foreach ( $option_keys as $display_key ) : 
                                $option_original_idx = $display_key; 
...
function anikeola_cbt_process_exam_submission() { 
    ...
    // Get the actual question IDs that were presented to the student
    // This is important if we implement "Number of Questions to Display"
    $all_question_ids_for_exam = get_post_meta( $exam_id, '_anikeola_cbt_exam_question_ids', true );
    if ( ! is_array( $all_question_ids_for_exam ) || empty( $all_question_ids_for_exam ) ) {
        wp_send_json_error( array( 'message' => __( 'Exam has no questions configured.', 'anikeola-cbt' ) ) ); return;
    }

    $num_questions_to_display_setting = get_post_meta( $exam_id, '_anikeola_cbt_num_questions_to_display', true );
    $num_questions_to_display = !empty($num_questions_to_display_setting) && is_numeric($num_questions_to_display_setting) ? intval($num_questions_to_display_setting) : 0;
    
    $randomize_questions_setting = get_post_meta( $exam_id, '_anikeola_cbt_randomize_questions', true );

    // Determine the actual set of questions the student saw
    // This logic needs to be robust if we want to store exactly which questions were presented in a randomized, limited set.
    // For now, we score against all questions that *could* have been part of the exam if randomized & limited.
    // A more advanced approach would store the exact `presented_question_ids` with the student's attempt.
    // For simplicity in this iteration, we'll score based on the submitted answers against their correct versions.
    // The $total_questions for percentage calculation should ideally be the number of questions *presented*.
    
    $questions_presented_to_student = array_keys($submitted_answers); // This gives us the IDs of questions the student answered
    $total_questions_for_scoring = count($questions_presented_to_student);
    if ($total_questions_for_scoring === 0 && !empty($all_question_ids_for_exam)) {
         // If no answers submitted, but exam has questions, assume they saw all (or the limited set)
        if ($num_questions_to_display > 0 && $num_questions_to_display < count($all_question_ids_for_exam)) {
            $total_questions_for_scoring = $num_questions_to_display;
        } else {
            $total_questions_for_scoring = count($all_question_ids_for_exam);
        }
    }
    if ($total_questions_for_scoring === 0) $total_questions_for_scoring = 1; // Avoid division by zero if no questions at all


    $score = 0; 
    $processed_answers_for_storage = array(); 
    
    foreach ( $submitted_answers as $question_id => $student_answer_index_str ) {
        ...
        $student_answer_index = intval( $student_answer_index_str ); 
    ...
    $percentage = ($total_questions_for_scoring > 0) ? round( ( $score / $total_questions_for_scoring ) * 100, 2 ) : 0;
    ...
    $result_data = array('exam_id' => $exam_id, 'user_id' => $user_id, 'score' => $score, 'total_questions' => $total_questions_for_scoring, 'percentage' => $percentage, 'passed' => $passed, 'submitted_answers' => $processed_answers_for_storage, 'timestamp' => $attempt_timestamp, 'ip_address' => $_SERVER['REMOTE_ADDR'] );
    ...
    wp_send_json_success( array('score' => $score, 'total_questions' => $total_questions_for_scoring, 'percentage' => $percentage, 'passed' => $passed, 'feedback_message'  => __( 'Your exam has been submitted and scored!', 'anikeola-cbt' ) ) );
}
